  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between
                  bg-canvas border-b border-[rgba(28,28,26,0.1)] transition-[padding] duration-300
                  ${scrolled ? 'px-[60px] py-3.5' : 'px-[60px] py-5'}`}
    >
      <span
        className="font-serif text-2xl font-semibold text-charcoal cursor-pointer tracking-wide"
        onClick={() => navigate('/')}
      >
      ...
      <div className="flex items-center gap-9">
        ...
        <NavLink
          to="/donate"
          className="bg-green text-cream px-6 py-2.5 text-[0.78rem] tracking-[0.1em]
                     uppercase rounded-sm transition-colors duration-200 no-underline
